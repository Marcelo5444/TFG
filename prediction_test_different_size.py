...
def WholeTest(args, model, size=1.0):
    if args.cuda:
        ...
        net = model 
    net.eval()
    saved_state_dict = torch.load(args.resume,map_location=lambda storage, loc: storage)
    ...
    for i in img_list:
        ...
        with torch.no_grad():
            if args.rgb:
                img = transform_rgb(i,args.cuda)
            else:
                img = transform(i,args.cuda)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default="/home/mlrig/Documents/TFG/Code/Code Repo/out/images/",
                        help='output directory of the model_test, for saving the seg_models')
    parser.add_argument("--resume", type=str, default="/home/mlrig/Documents/TFG/Code/weights.pth")
