...
from libs.core.loss import CriterionOhemDSN, CriterionDSN, CriterionICNet, CriterionDFANet, OhemCrossEntropy2dTensor
...
def get_arguments():
    ...
    parser.add_argument("--learning_rate", type=float, default=900,
    ...
    parser.add_argument('--apex', action='store_false', default=False,
    ...
    parser.add_argument("--epochs", default=20, type=int, help="num of iters")
...
"""
    ...
    torch.distributed.init_process_group(backend='nccl',
                                         init_method='env://',rank=args.local_rank,world_size=1)"""
    # set optimizer
    ...
    saved_state_dict = torch.load("/home/mlrig/Documents/TFG/Code/weights.pth")
    ...
    if args.ohem:
        ...
        #criterion = smp.utils.losses.BCEWithLogitsLoss()  # CrossEntropy
        pass
    criterion = OhemCrossEntropy2dTensor(ignore_label = args.ignore_label) 
    criterion.cuda()
    ...
    for j in range(args.epochs):
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            total_loss = loss.data.cpu().numpy() + total_loss
            loss.backward()
            optimizer.step()
        ...
        if True:
                                        Log.info('iter = {} of {} completed, lr={}, loss = {}'.format(i_iter,
                                                                                                  len(trainloader), lr,total_loss/len(trainloader)))
                                        #if i_iter % args.save_pred_every == 0 and i_iter > args.save_start:
                                        print('save models ...')
                                        torch.save(model.state_dict(),'/home/mlrig/Documents/TFG/Code/weights.pth')
